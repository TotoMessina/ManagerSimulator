...
import { PartidoCopa, TablaCopa } from '../types';

export const CopaInternacionalView: React.FC = () => {
  const { copaCampeones, equipos, equipoUsuario, fechaActual } = useGame();
  ...
  const [grupoSeleccionado, setGrupoSeleccionado] = useState<'A' | 'B' | 'C'>('A');
  ...
  if (!copaCampeones) {
    return (
      <div className="bg-slate-900/40 border border-slate-800/80 rounded-2xl p-12 text-center max-w-lg mx-auto mt-12 shadow-lg backdrop-blur-md">
        ...
        <h3 className="text-lg font-bold text-slate-350 mb-2">Copa de Campeones No Activa</h3>
        <p className="text-slate-500 text-xs leading-relaxed">
          La competencia internacional de élite no está programada en este momento. Volverá a sortearse en la próxima temporada.
        </p>
      </div>
    );
  }
  ...
  const obtenerBanderaEquipo = (id: string): string => {
    if (['real-madrid', 'barcelona', 'atletico-madrid', 'sevilla', 'real-sociedad', 'athletic-bilbao', 'villarreal', 'betis', 'valencia', 'osasuna'].includes(id)) return '🇪🇸';
    if (['manchester-city', 'arsenal', 'liverpool', 'chelsea', 'manchester-united', 'tottenham', 'newcastle', 'aston-villa', 'brighton', 'west-ham'].includes(id)) return '🏴󠁧󠁢󠁥󠁮󠁧󠁿';
    if (['inter-milan', 'ac-milan', 'juventus', 'napoli', 'roma', 'lazio', 'atalanta', 'fiorentina', 'torino', 'bologna'].includes(id)) return '🇮🇹';
    return '🇪🇺';
  };

  const { grupos, faseActual, partidosGrupos, semifinales, final, campeon } = copaCampeones;
  ...
  return (
    <div className="space-y-6">
      ...
      <div 
        ...
        style={{
          background: 'linear-gradient(135deg, rgba(30, 27, 75, 0.9) 0%, rgba(15, 12, 45, 0.95) 100%)',
      ...
      >
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative z-10">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/25 text-indigo-300 font-extrabold text-[10px] tracking-widest uppercase">
              ⭐ UEFA Copa de Campeones
            </div>
            <h1 className="text-3xl font-extrabold tracking-tight text-white mt-3">
              LA ÉLITE DEL FÚTBOL CONTINENTAL
            </h1>
            <p className="text-xs text-slate-350 max-w-xl mt-1.5 leading-relaxed">
              Los 4 mejores clubes de España, Inglaterra e Italia compiten en una batalla de poder a poder. ¿Tenés la capacidad táctica para levantar la Orejona y consagrarte en la historia europea?
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                  ...
                  <button
                    ...
                    className={`w-10 h-10 rounded-lg text-xs font-extrabold transition-all border ${
                      grupoSeleccionado === gId
                        ? 'bg-indigo-600 border-indigo-500 text-white font-black shadow-inner shadow-indigo-700'
              ...
              return (
                <div className="bg-slate-900/40 border border-slate-800/80 rounded-2xl overflow-hidden shadow-lg backdrop-blur-md">
                  ...
                  <div className="p-3.5 bg-slate-950/40 border-t border-slate-850 text-[10px] text-slate-500 flex justify-between">
                    <span>🟢 Los 2 primeros califican al ranking de semifinalistas</span>
          ...
          <div className="bg-slate-900/40 border border-slate-800/80 rounded-2xl p-5 flex flex-col justify-between shadow-lg backdrop-blur-md min-h-[300px]">
                  ...
                  return jfGroup.partidos.map((partido, pIdx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Eventos / Penales en caso de empate (solo si existiesen, no en fase de grupos) */}
                      </div>
                    );
                  });
                })}
            ...
            <div className="text-[10px] text-slate-500 border-t border-slate-850 pt-3 leading-normal mt-6 font-medium">
              💡 Los grupos juegan los miércoles intercalados. El DT usuario dirige sus partidos continentalmente cuando su club tiene cita europea.
            </div>
          </div>
      ...
      {activeTab === 'eliminatorias' && (
        <div className="space-y-8 py-4">
          ...
          {/* Brackets Layout */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center max-w-5xl mx-auto relative">
            
            {/* SEMIFINALES */}
            <div className="space-y-6 md:col-span-1">
              <h3 className="text-xs font-extrabold text-indigo-400 uppercase tracking-widest text-center mb-4 border-b border-indigo-500/20 pb-1.5">
                Semifinales
              </h3>
              ...
              {semifinales ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-between items-center text-xs">
                          <div className="flex items-center gap-1.5 font-bold truncate">
                            ...
                            <span className="truncate text-slate-200">{localNombre}</span>
                            {esLocalUsuario && <span className="text-[7px] bg-indigo-650 text-white font-extrabold px-1 rounded uppercase">Tú</span>}
                          </div>
                          <span className="font-black font-mono text-slate-100">
                        ...
                        <div className="flex justify-between items-center text-xs">
                          <div className="flex items-center gap-1.5 font-bold truncate">
                            ...
                            <span className="truncate text-slate-200">{visitanteNombre}</span>
                            {esVisitanteUsuario && <span className="text-[7px] bg-indigo-650 text-white font-extrabold px-1 rounded uppercase">Tú</span>}
                          </div>
                          <span className="font-black font-mono text-slate-100">
                      ...
                      {esJugado && partido.penalesLocal !== undefined && partido.penalesVisitante !== undefined && (
                        <div className="bg-slate-950/80 px-2 py-1 rounded border border-slate-850 text-[10px] text-center font-bold text-indigo-400 font-mono">
                ...
                <div className="bg-slate-900/30 border border-slate-850 border-dashed rounded-2xl p-6 text-center text-slate-500 italic text-xs leading-relaxed">
                  Semifinales pendientes. Se definirán al finalizar la fase de grupos.
                </div>
              )}
            </div>

            {/* CONECTOR GRÁFICO (PARA ESCRITORIOS) */}
            <div className="hidden md:flex flex-col items-center justify-center text-indigo-600 text-3xl font-black md:col-span-1 select-none pointer-events-none">
              <div>➔</div>
              <div className="text-[10px] uppercase font-extrabold tracking-widest text-slate-600 mt-2 font-sans">Camino a la Final</div>
              <div className="mt-2">⬅</div>
            </div>

            {/* GRAN FINAL */}
            <div className="space-y-6 md:col-span-1">
              <h3 className="text-xs font-extrabold text-amber-500 uppercase tracking-widest text-center mb-4 border-b border-amber-500/20 pb-1.5 animate-pulse">
                Gran Final
              </h3>
              ...
              {final ? (
                (() => {
                  ...
                  return (
                    <div 
                      className={`p-5 rounded-2xl border-2 flex flex-col justify-between space-y-4 relative overflow-hidden transition-all ${
                        esPartidoDT 
                    ...
                    >
                      ...
                      <div className="space-y-2.5">
                        ...
                        <div className="flex justify-between items-center text-xs">
                          <div className="flex items-center gap-1.5 font-bold truncate">
                            <span className="text-xl">{localEscudo}</span>
                            <span className="truncate text-slate-200">{localNombre}</span>
                            {esLocalUsuario && <span className="text-[7px] bg-indigo-650 text-white font-extrabold px-1 rounded uppercase">Tú</span>}
                          </div>
                          <span className="font-black font-mono text-slate-100 text-sm">
                            {esJugado ? partido.golesLocal : '-'}
                          </span>
                        </div>
                        ...
                        <div className="flex justify-between items-center text-xs">
                          <div className="flex items-center gap-1.5 font-bold truncate">
                            <span className="text-xl">{visitanteEscudo}</span>
                            <span className="truncate text-slate-200">{visitanteNombre}</span>
                            {esVisitanteUsuario && <span className="text-[7px] bg-indigo-650 text-white font-extrabold px-1 rounded uppercase">Tú</span>}
                          </div>
                          <span className="font-black font-mono text-slate-100 text-sm">
                      ...
                      {/* Penales de desempate */}
                      {esJugado && partido.penalesLocal !== undefined && partido.penalesVisitante !== undefined && (
                        <div className="bg-amber-950/20 px-2 py-1 rounded border border-amber-500/30 text-[10px] text-center font-bold text-amber-400 font-mono">
                          Penales: {partido.penalesLocal} - {partido.penalesVisitante}
                        </div>
                      )}

                      {esJugado && (
                        <div className="pt-2 border-t border-slate-850/80 text-center text-[10px] font-black text-white uppercase tracking-wider bg-slate-950/65 py-1.5 rounded-lg border border-slate-900">
                ...
                <div className="bg-slate-900/30 border border-slate-850 border-dashed rounded-2xl p-6 text-center text-slate-500 italic text-xs leading-relaxed">
                  Gran Final continental pendiente. Se programará luego de resolverse las Semifinales.
                </div>
              )}
            </div>
          ...
          {/* Pedestal De Honor */}
          {campeon && (
            <div className="bg-gradient-to-r from-amber-500/5 via-yellow-500/10 to-amber-500/5 border border-yellow-500/25 max-w-xl mx-auto rounded-2xl p-6 text-center shadow-lg relative overflow-hidden animate-fade-in">
              ...
              <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                El club {obtenerNombreEquipo(campeon)} se ha consagrado monarca absoluto del continente tras una trayectoria europea impecable.
              </p>
